...
def show_help():
    message = r"""
            ...
            -You can copy paste all the links of the doujins you want to download into a text file and set
             the value of batch in the thread3 to the path of this text file.
            Example: 
            ...
            Note: The batch line in the thread3 will be reset every time the script is executed.
...
def update_config(cmd): #Update existing/add new parameters in config from console using set command
    ...
    f = open('thread3').read()
    if re.findall(rf'{cmd[0]} = ".*"', f): #If parameter exists in thread3, update existing
        content = re.sub(rf'{cmd[0]} = ".*"', rf'{cmd[0]} = "{cmd[1]}"', f)
    else: #Otherwise add new parameter
        content = f + f'\n{cmd[0]} = "{cmd[1]}"'
    open('thread3', 'w').write(content)
    config[cmd[0]] = cmd[1] #Updates value in the already read config variable
    print('Done Successfully')

def parse_config(): #Parse and check parameters in thread3 into a dict called config
    f = open('thread3').read()
    ...
    if config["batch"] is None:
        ...
        if os.path.exists(config['batch']):
            ...
            # Cleanup
            # open('thread3', 'w').write(re.sub(r'batch = ".*"', r'batch = ""', f))
        else:
...
if __name__ == "__main__":
    ...
    if os.path.exists('thread3'):
        config = parse_config()
    else:
        open('thread3', 'w').write(default_config)
        config = parse_config()
